...
public class User extends Pluginable {
     ...
     *
     * @param bot The bot the user is linked to
     * @param uniqueId The generated unique IRC id.
     */
    public User(QoreBot bot, String uniqueId) {
        ...
        PreparedStatement st = Database.gps("SELECT id, username, level FROM users " +
                "WHERE last_unique_id = ? LIMIT 0,1");
     ...
     *
     * Several other warnings are not misplaced: this user is identified, but
     * it is NOT identified based on the !identify-command nor the last known
     ...
     *             linked to something.
     * @param username The user id to identify.
     */
    public User(String username) {
        ...
        PreparedStatement st = Database.gps("SELECT id, level FROM users " +
     ...
     *
     * @param username The username to identify for.
     * @param password The password to identify with.
     * @return True if the user was successfully identified. Please note that
     *   a result of false does not mean that the user is not identified.
     */
    ...
    public boolean checkPassword(String password) {
        ...
        PreparedStatement st = Database.gps("SELECT id FROM users " +
                "WHERE username = ? AND password = ? LIMIT 0,1");
     ...
     * @return False if either the connection failed, the old password is invalid
     *    or the current user is not identified.
     */
     ...
     * is done in the database. This might cause some non-errors to appear in the
     * log file.
     *
     * The checks should be something along the lines of unique usernames,
     * unique last IDs and non-empty passwords (that last one is not done either).
     *
    ...
    /**
     * Changes the unique id of the current user. Should not be called unless there's
     ...
     *
     * @param uniqueId The new unique identifier
     * @param updateLastUniqueId If true, tries to update the database field with
     *                           the last known unique id of the identified curr. user.
     * @return true if the change succeeded.
     */
     ...
     * not be set.
     *
     * @param uniqueId The uniqueId to clear
    ...
    /**
     * Retrieves the user level for this channel. If the user level is unknown or
     * lower then the user minimum, the user minimum is returned.
     *
     * @param channel The channel to retrieve the information for.
    ...
    public UserLevel getLevel(Channel channel) {
        ...
        PreparedStatement st = Database.gps("SELECT level FROM channels_users " +
                    "WHERE channel_id = ? AND user_id = ? LIMIT 0,1");
     ...
     * 
     * @param level The new user level. Should be a constant.
     ...
     *
     * @param channel The channel to set the data for. Should not be null.
     * @param level The new user level. Should be one of the constants.
     * @return true when the update succeeds
     */
    public boolean setLevel(Channel channel, UserLevel level) {
        ...
        PreparedStatement st = Database.gps("REPLACE INTO channels_users " +
                "SET channel_id = ?, user_id = ?, level = ?");
        if (st == null)
            return false;
    ...
    public boolean removeLevel(Channel channel) {
        ...
        PreparedStatement st = Database.gps("DELETE FROM channels_users " +
    ...
    /**
     * Tries to determine the level from a given string. The string can be either
     * a number representing the integer value of the level, or the string representation
     * returned by {@link UserLevel#toString()}
     *
     * @param level The string representation to convert. Must not be null.
    ...
    // These are called when the bot RECEIVES a message from this User:

    /*
    public void onAction(String action) {
        this.update(new Event(Event.USER_ONACTION, this, action));
    }
    public void onNickChange(String oldNick, String newNick) {
        this.update(new Event(Event.USER_ONNICKCHANGE, this, oldNick, newNick));
    }
    public void onNotice(String notice) {
        this.update(new Event(Event.USER_ONNOTICE, this, notice));
    }
    public void onPrivateMessage(String message) {
        this.update(new Event(Event.USER_ONPRIVATEMESSAGE, this, message));
    }
    public void onQuit(String reason) {
        this.update(new Event(Event.USER_ONQUIT, this, reason));
    }
    public void onUserMode(String mode) {
        this.update(new Event(Event.USER_ONUSERMODE, this, mode));
    }*/
}