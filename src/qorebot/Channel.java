...
public class Channel extends Pluginable {
     ...
     *
     * @param bot The bot working on this channel. Should not be null.
     * @param id An unique integer identifying the channel.
     * @param name The name of the channel. Should not be null.
     */
     ...
     * @param id An unique integer identifying the channel.
     * @param name The name of the channel. Should not be null.
     * @param key The key for the channel. May be null.
     */
     ...
     * @param key The new key for this channel. May be null to unset.
     */
    public void setKey(String key) {
        this.key = key;

        PreparedStatement st = Database.gps("UPDATE channels SET key = ? " +
                "WHERE id = ?");
        try {
            if (st != null) {
                st.setString(1, key);
     ...
     *
     * @param key The key of the channel. Should not be null.
     */
     ...
     *
     * @param reason The reason to leave. Should not be null.
     */
     ...
     *
     * @see org.jibble.pircbot.PircBot#sendCTCPCommand(java.lang.String, java.lang.String)
     */
...
/*
    public void onAction(User sender, String action) {
        this.update(new Event(Event.CHANNEL_ONACTION, this, sender, action));
    }
    public void onChannelInfo(int userCount, String topic) {
        this.update(new Event(Event.CHANNEL_ONCHANNELINFO, this, userCount, topic));
    }
    public void onDeop(User source, String recipient) {
        this.update(new Event(Event.CHANNEL_ONDEOP, this, source, recipient));
    }
    public void onDeVoice(User source, String recipient) {
        this.update(new Event(Event.CHANNEL_ONDEVOICE, this, source, recipient));
    }
    public void onInvite(User source) {
        this.update(new Event(Event.CHANNEL_ONINVITE, this, source));
    }
    public void onJoin(User sender) {
        this.update(new Event(Event.CHANNEL_ONJOIN, this, sender));
    }
    public void onKick(User kicker, String recipientNick, String reason) {
        this.update(new Event(Event.CHANNEL_ONKICK, this, kicker, recipientNick, reason));
    }
    public void onMessage(User sender, String message) {
        this.update(new Event(Event.CHANNEL_ONMESSAGE, this, sender, message));
    }
    public void onMode(User source, String mode) {
        this.update(new Event(Event.CHANNEL_ONMODE, this, source ,mode));
    }
    public void onNickChange(User user, String oldNick, String newNick) {
        this.update(new Event(Event.CHANNEL_ONNICKCHANGE, this, user, oldNick, newNick));
    }
    public void onNotice(User source, String notice) {
        this.update(new Event(Event.CHANNEL_ONNOTICE, this, source, notice));
    }
    public void onOp(User source, String recipient) {
        this.update(new Event(Event.CHANNEL_ONOP, this, source, recipient));
    }
    public void onPart(User sender) {
        this.update(new Event(Event.CHANNEL_ONPART, this, sender));
    }
    public void onQuit(User source, String reason) {
        this.update(new Event(Event.CHANNEL_ONQUIT, this, source, reason));
    }
    public void onRemoveChannelBan(User source, String hostmask) {
        this.update(new Event(Event.CHANNEL_ONREMOVECHANNELBAN, this, source, hostmask));
    }
    public void onRemoveChannelKey(User source, String key) {
        this.update(new Event(Event.CHANNEL_ONREMOVECHANNELKEY, this, source, key));
    }
    public void onRemoveChannelLimit(User source) {
        this.update(new Event(Event.CHANNEL_ONREMOVECHANNELLIMIT, this, source));
    }
    public void onRemoveInviteOnly(User source) {
        this.update(new Event(Event.CHANNEL_ONREMOVEINVITEONLY, this, source));
    }
    public void onRemoveModerated(User source) {
        this.update(new Event(Event.CHANNEL_ONREMOVEMODERATED, this, source));
    }
    public void onRemoveNoExternalMessages(User source) {
        this.update(new Event(Event.CHANNEL_ONREMOVENOEXTERNALMESSAGES, this, source));
    }
    public void onRemovePrivate(User source) {
        this.update(new Event(Event.CHANNEL_ONREMOVEPRIVATE, this, source));
    }
    public void onRemoveSecret(User source) {
        this.update(new Event(Event.CHANNEL_ONREMOVESECRET, this, source));
    }
    public void onRemoveTopicProtection(User source) {
        this.update(new Event(Event.CHANNEL_ONRREMOVETOPICPROTECTION, this, source));
    }
    public void onSetChannelBan(User source, String hostmask) {
        this.update(new Event(Event.CHANNEL_ONSETCHANNELBAN, this, source, hostmask));
    }
    public void onSetChannelKey(User source, String key) {
        this.update(new Event(Event.CHANNEL_ONSETCHANNELKEY, this, source, key));
    }
    public void onSetChannelLimit(User source, int limit) {
        this.update(new Event(Event.CHANNEL_ONSETCHANNELLIMIT, this, source, limit));
    }
    public void onSetInviteOnly(User source) {
        this.update(new Event(Event.CHANNEL_ONSETINVITEONLY, this, source));
    }
    public void onSetModerated(User source) {
        this.update(new Event(Event.CHANNEL_ONSETMODERATED, this, source));
    }
    public void onSetNoExternalMessages(User source) {
        this.update(new Event(Event.CHANNEL_ONSETNOEXTERNALMESSAGES, this, source));
    }
    public void onSetPrivate(User source) {
        this.update(new Event(Event.CHANNEL_ONSETPRIVATE, this, source));
    }
    public void onSetSecret(User source) {
        this.update(new Event(Event.CHANNEL_ONSETSECRET, this, source));
    }
    public void onSetTopicProtection(User source) {
        this.update(new Event(Event.CHANNEL_ONSETTOPICPROTECTION, this, source));
    }
    public void onTopic(String topic, String setBy, long date, boolean changed) {
        this.update(new Event(Event.CHANNEL_ONTOPIC, this, topic, setBy, date, changed));
    }
    public void onUserList(org.jibble.pircbot.User[] users) {
        this.update(new Event(Event.CHANNEL_ONUSERLIST, this, users));
    }
    public void onVoice(User source, String recipient) {
        this.update(new Event(Event.CHANNEL_ONVOICE, this, source, recipient));
    }*/

    @Override
    public boolean equals(Object o) {
        return (o instanceof Channel
                && ((Channel) o).getName().equals(this.getName())
                && ((Channel) o).getBot() == this.getBot());
    }
